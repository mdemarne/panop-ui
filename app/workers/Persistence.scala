 ...
 */
class Persistence(id: String, master: ActorRef) extends Actor
with ActorLogging {
  import com._
      ...
      (master !? com.AskProgress, master !? com.AskResults) match {
        case (pp: com.AswProgress, pr: com.AswResults) =>
          pr.results.foreach {res =>
            if (Try(MResult.store(id, res.search.url.link,
              com.Query.printNormalForm(res.matches))
            ).isFailure) {
              log.error("Could not save results") /* TODO: better loggin */
            }
          }
        if (pp.percent < 1.0 || pp.nbExplored == 1) {
          self >! ("persist", Settings.updateRate)
