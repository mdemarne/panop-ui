...
case class MResult(id: String, datetime: DateTime, url: String, matches: String)
  ...
  val lineParser = {
    get[String](s"${this.tableName}.id") ~
      ...
      get[String](s"${this.tableName}.matches") map {
        case id ~ datetime ~ url ~ matches => MResult(id, datetime, url, matches)
      }
  }

  def store(id: String, url: String, matches: String): Unit = DB.withTransaction { implicit conn =>
    if (opt('id -> id, 'url -> url).isEmpty) insert('id -> id, 'datetime -> new DateTime(), 'url -> url, 'matches -> matches)().commitDirect
  }
  def fetchById(id: String): List[MResult] = select('id -> id).toList

  def clean(datetime: DateTime) = DbTransaction { implicit connection =>
    SQL(s"""DELETE FROM ${this.tableName} WHERE datetime < {datetime}""").on('datetime -> datetime).executeUpdate()
  }
}
